...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "en": {
        "app.subtitle": "One button to get ready. Then pick a folder and say what you want.",
        "label.language": "Language",
        "section.system": "1. Get ready",
        "section.project": "2. Pick a project folder",
        "section.task": "3. Describe your task",
        ...
        "status.simple_checking_hint": "FirstWindow is checking whether everything needed for a safe $0 start is ready.",
        "status.simple_probe": "Testing the route once. Start unlocks when the check passes.",
        ...
        "status.simple_verify_hint": "Setup is almost done. Press Make Me Ready to finish the check.",
        "status.simple_setup": "Setup needed",
        "status.simple_setup_hint": "Press Make Me Ready. FirstWindow handles setup and only asks when your approval or API key is required.",
        ...
        "next.ready": "Setup is ready. Press Make Me Ready once to verify it, then pick a folder and Start.",
        "next.install": "Press Make Me Ready. FirstWindow can install Hermes after one approval.",
        "next.configure": "A local model is not ready. Press Make Me Ready for the exact next step.",
        "dialog.setup": "Setup",
    ...
    "zh-CN": {
        "app.subtitle": "点一次“一键就绪”，再选文件夹、说任务，就可以开始。",
        "label.language": "语言",
        "section.system": "1. 准备好",
        ...
        "status.simple_checking_hint": "FirstWindow 正在检查安全启动所需的内容是否已经准备好。",
        "status.simple_probe": "正在做一次真实通道检查。通过后“开始”会自动解锁。",
        "status.simple_ready": "已准备好",
        "status.simple_ready_hint": "通道已验证。选择项目文件夹，描述任务，然后点击“开始”。",
        "status.simple_verify": "还差一次验证",
        "status.simple_verify_hint": "设置已经接近完成。点击“一键就绪”完成最后检查。",
        "status.simple_setup": "需要先设置",
        "status.simple_setup_hint": "点击“一键就绪”。FirstWindow 会自动处理设置，只有需要你确认安装或输入 API Key 时才会询问。",
        ...
        "status.summary": "Agnes：{agnes}\nHermes：{hermes}\n{guard}",
        "next.ready": "设置已经准备好。点击“一键就绪”完成一次验证，然后选文件夹并开始。",
        "next.install": "点击“一键就绪”。FirstWindow 会在你确认一次后安装 Hermes。",
        "next.configure": "本地模型还没准备好。点击“一键就绪”查看准确的下一步。",
        "dialog.setup": "设置",
        "dialog.project": "项目",
        "dialog.task": "任务",
        "dialog.runtime": "运行通道",
...
TRANSLATIONS["en"].update({
    ...
    "next.agnes_key_missing": "One thing is needed: your Agnes API key. Press Make Me Ready and paste it when asked. A verified local route can be used instead.",
    "next.confirm_agnes": "Your Agnes cloud route is ready to check. Confirm this account/key is $0 for you, then FirstWindow tests it.",
    "next.prepare_agnes": "Press Make Me Ready. FirstWindow will prepare the Agnes cloud route safely.",
    "confirm.agnes_free.title": "Confirm $0 Agnes route",
    "confirm.agnes_free": "Agnes advertises a Free API, but account billing can vary. Confirm only if this API key/account route is $0 for you. FirstWindow will block unknown-cost fallback.",
    "setup.agnes_profile_preparing": "Preparing the Agnes cloud route safely…",
    "setup.agnes_profile_ready": "Agnes cloud route prepared. Continuing setup…",
    "setup.agnes_profile_failed": "Could not prepare the isolated Agnes API profile: {reason}",
    "setup.agnes_key_title": "Agnes API key needed",
    "setup.agnes_key_prompt": "Paste your Agnes API key. If you do not have one, get it from the official Agnes API/account page first: https://agnes-ai.com/ . FirstWindow stores it only in its isolated Hermes profile and never copies keys from your default profile.",
    "setup.agnes_key_saved": "API key saved to FirstWindow's isolated profile. Continuing setup…",
    ...
    "error.readiness_required": "Setup is not verified yet. Press Make Me Ready first; FirstWindow will not start until the real check passes.",
})
TRANSLATIONS["zh-CN"].update({
    ...
    "next.agnes_key_missing": '还差一项：你的 Agnes API Key。点击“一键就绪”，按提示粘贴即可；如果已有验证通过的本地通道，也可以直接使用本地通道。',
    "next.confirm_agnes": 'Agnes 云端通道已经可以检查。确认这个账号/Key 对你是 $0 后，FirstWindow 会自动测试。',
    "next.prepare_agnes": '点击“一键就绪”。FirstWindow 会安全准备 Agnes 云端通道。',
    "confirm.agnes_free.title": '确认 $0 Agnes 通道',
    "confirm.agnes_free": 'Agnes 官方展示 Free API，但不同账号的计费状态可能不同。只有确认这个 API Key/账号对你确实是 $0 时才继续。FirstWindow 会阻止未知成本的 fallback。',
    "setup.agnes_profile_preparing": '正在安全准备 Agnes 云端通道…',
    "setup.agnes_profile_ready": 'Agnes 云端通道已经准备好，正在继续设置…',
    "setup.agnes_profile_failed": '无法准备 Agnes API 隔离配置：{reason}',
    "setup.agnes_key_title": '需要 Agnes API Key',
    "setup.agnes_key_prompt": '粘贴你的 Agnes API Key。如果还没有，请先到 Agnes 官方 API/账号页面获取：https://agnes-ai.com/ 。FirstWindow 只会把它保存到自己的隔离 Hermes profile，不会从你的默认 profile 复制其他 Key。',
    "setup.agnes_key_saved": 'API Key 已保存到 FirstWindow 的隔离 profile，正在继续设置…',
    ...
    "error.readiness_required": '设置还没有通过验证。请先点击“一键就绪”；真实检查通过前 FirstWindow 不会启动任务。',
})
